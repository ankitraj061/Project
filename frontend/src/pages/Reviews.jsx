      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8">
          ...
          <div className="bg-white px-6 py-3 rounded-full shadow-md flex items-center space-x-3">
            <div className="text-3xl font-bold text-indigo-600">{averageRating}</div>
            <div className="flex items-center">
              {[...Array(5)].map((_, i) => (
                <Star 
                  key={i} 
                  size={20} 
                  fill={i < Math.round(parseFloat(averageRating)) ? "#4F46E5" : "none"} 
                  color={i < Math.round(parseFloat(averageRating)) ? "#4F46E5" : "#CBD5E1"} 
                  className="mr-1" 
                />
              ))}
            </div>
