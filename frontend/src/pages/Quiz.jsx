...
function SkincareQuiz() {
  ...
  const getRecommendation = () => {
    ...
    const oilinessAnswer = answers[1];
    const sensitivityAnswer = answers[2];
    const poreAnswer = answers[3];
    const concernAnswer = answers[4];
    ...
    if (sensitivityAnswer?.includes("Often")) {
      products = products.map(p => p.replace(/^/, "Sensitive "));
  ...
  const handleAnswer = (option) => {
    ...
    if (step < questions.length) {
      ...
      const skinTypeKey = getSkinTypeKey(recommendationResult.skinType);
      const filtered = myProducts.filter(product => {
        ...
        if (product.skinType && Array.isArray(product.skinType)) {
          return product.skinType.includes(skinTypeKey);
        }
        // If no skinType field, check category or tags
        if (product.category || product.tags) {
          return true; // Show all products if no specific skin type filtering
        }
        return false;
      });
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-800 mb-2">
              Recommended Products for {name}
            </h1>
            <p className="text-lg text-gray-600">Curated specifically for your {recommendation.skinType.toLowerCase()}</p>
          </div>
          ...
          {availableCategories.length > 0 && (
            <div className="flex flex-wrap justify-center gap-2 mb-8">
              ...
              {availableCategories.map(category => {
                const count = filteredProducts.filter(p => p.category?.toLowerCase() === category.toLowerCase()).length;
                return (
                  <button
                    key={category}
        ...
        <div className="max-w-4xl w-full">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            ...
            >
              ...
              View Suggested Products ({filteredProducts.length})
            </button>
